...
public class EditableCondition extends BasicEditable {
	public static final UUID _modeluuid = UUID.fromString("23a72f18-5c90-11dc-8314-0800200c9a66");
	public static final String _displayname = "Condition";
	...
	private LinkedList<EditableEvent> out;
	private LinkedList<EditableEvent> in;
	
	public LinkedList<EditableEvent> getOut() {
		return (LinkedList<EditableEvent>)out.clone();
	}

	public LinkedList<EditableEvent> getIn() {
		return (LinkedList<EditableEvent>)in.clone();
	}

	public void removeIn(EditableEvent t) {
		in.remove(t);
	}

	public void removeOut(EditableEvent t) {
		out.remove(t);
	}
	...
	public boolean addIn(DefaultConnection con) {
		EditableEvent t = (EditableEvent)con.getFirst();
		if (in.contains(t))
			return false;
		in.add(t);
		connections.add(con);
	...
	public boolean addOut(DefaultConnection con) {
		EditableEvent t = (EditableEvent)con.getSecond();
		if (out.contains(t))
			return false;
		out.add(t);
		connections.add(con);
	...
	public EditableCondition(BasicEditable parent) throws UnsupportedComponentException {
		super(parent);
		boundingBox.setExtents(new Vec2(-0.05f, -0.05f), new Vec2(0.05f, 0.05f));
		token_count = 0;
		out = new LinkedList<EditableEvent>();
		in = new LinkedList<EditableEvent>();
	}
	...
	public static void setPlaceColor(Colorf placeColor) {
		EditableCondition.placeColor = placeColor;
	}
	...
	public static void setTokenColor(Colorf tokenColor) {
		EditableCondition.tokenColor = tokenColor;
	}
	...
	public static void setPlaceOutlineColor(Colorf placeOutlineColor) {
		EditableCondition.placeOutlineColor = placeOutlineColor;
	}
